...
\newpage
\section{Demarche scientifique et recherche : la sensibilisation au phishing produit-elle un effet durable ?}
...
\begin{center}
\small
\begin{tabular}{@{}p{4.6cm}p{2.0cm}p{2.0cm}p{2.0cm}p{2.0cm}@{}}
\toprule
\textbf{Groupe} & \textbf{T0} & \textbf{T+1 mois} & \textbf{T+3 mois} & \textbf{T+6 mois} \\
\midrule
...
\begin{center}
{\small\bfseries\color{apcblue} Evolution du taux de clic sur six mois}\\[6pt]
\begin{tikzpicture}[x=2.6cm,y=0.055cm]
\draw[->] (0,0) -- (0,40) node[above,font=\scriptsize]{Taux de clic (\%)};
\draw[->] (0,0) -- (3.3,0) node[right,font=\scriptsize]{Temps};
\foreach \x/\lbl in {0/T0, 1/T+1 mois, 2/T+3 mois, 3/T+6 mois}
  \node[font=\scriptsize,below] at (\x,-1.5) {\lbl};
\draw[apcblue,thick,mark=*,mark options={fill=apcblue}] plot coordinates {(0,33) (1,11) (2,21) (3,29)};
\draw[apcteal,thick,mark=*,mark options={fill=apcteal}] plot coordinates {(0,35) (1,10) (2,12) (3,13)};
\node[apcblue,font=\scriptsize,right] at (3.05,29) {Groupe A};
\node[apcteal,font=\scriptsize,right] at (3.05,13) {Groupe B};
\end{tikzpicture}
\end{center}
\vspace{4pt}

Le groupe A, qui reproduit le format de la mission 7, montre exactement la meme dynamique que celle observee lors de cette mission jusqu'a trois mois (33\,\% puis 11\,\%), mais la mesure a six mois, que je n'avais pas realisee en mission 7, revele une remontee nette du taux de clic a 29\,\%, proche du niveau initial. Le groupe B, qui a beneficie d'un rappel mensuel, conserve au contraire un taux de clic bas et stable jusqu'a six mois (13\,\%).
...
\textbf{Contributions.} Ce chapitre a permis de prolonger, dans le temps, la mesure ponctuelle realisee en mission 7, en mettant en evidence une remontee du taux de clic a six mois en l'absence de rappel, et en montrant qu'un rappel mensuel tres leger suffit a maintenir un effet de sensibilisation nettement plus stable.

\textbf{Limites.} Ce travail reste une exploration personnelle et modeste : echantillon restreint et non representatif d'un client reel, repartition des groupes non aleatoire, un seul indicateur mesure, et duree d'observation limitee a six mois.

\textbf{Travaux futurs.} Je souhaiterais, si l'occasion se presente, pouvoir reconduire ce protocole a une echelle plus large, avec l'accord explicite d'un client et sous la supervision de mon tuteur, en testant plusieurs frequences de rappel afin d'identifier un optimum entre efficacite et lassitude des utilisateurs, et en prolongeant la mesure au-dela de six mois pour verifier la stabilite du dispositif dans la duree.
